...
for ng in range(args.num_generator):
    ...
    testg = gan.Generator(source=tg.net, mode='test', **gen_kwargs).data
...
g_updates = []
for tg in train_gen:
    g_updates.extend(tg.net.get_updates(updater=updater))

for gi, train_g in enumerate(train_g):
    disp_costs["G%d" % gi] = disp(train_g.net.get_loss())
